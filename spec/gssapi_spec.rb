...
module PacketGen::Plugin
  describe GSSAPI do
    ...
    let(:pkts2) { read_packets('smb2-nego.pcapng') }
    ...
    it 'parses legacy initial context blob' do
      gssapi = pkts2[7].smb2_negotiate_response.buffer
      expect(gssapi).to be_a(GSSAPI)
      expect(gssapi.chosen).to eq(0)
      expect(gssapi[:oid].value).to eq('1.3.6.1.5.5.2')
      expect(gssapi[:token_init][:mech_types].value.size).to eq(5)
      expect(gssapi[:token_init][:mech_types][0].value).to eq('1.3.6.1.4.1.311.2.2.30')
      expect(gssapi[:token_init][:mech_types][1].value).to eq('1.2.840.48018.1.2.2')
      expect(gssapi[:token_init][:mech_types][2].value).to eq('1.2.840.113554.1.2.2')
      expect(gssapi[:token_init][:mech_types][3].value).to eq('1.2.840.113554.1.2.2.3')
      expect(gssapi[:token_init][:mech_types][4].value).to eq('1.3.6.1.4.1.311.2.2.10')
      str = force_binary("\xA3\e0\x19\xA0\x17\e\x15Server2008@SMB3.local")
      expect(gssapi[:token_init][:mech_list_mic].value).to eq(str)
    end
